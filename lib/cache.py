...
cache_path = utils.translate_path('special://temp/%s/cache/' % utils.get_id())
try:
    if not os.path.exists(cache_path):
        os.makedirs(cache_path)
except Exception as e:
    utils.log('Failed to create cache: %s: %s' % (cache_path, e))

cache_enabled = config.X.CACHE_ENABLED
...
def reset_cache():
    try:
        ...
        make_cache_path()
        utils.log('Cache reset')
        return True
    except Exception as e:
        utils.log('Failed to Reset Cache: %s' % (e))
        return False
...
def _save_func(name, args=None, kwargs=None, result=None):
    try:
        ...
        utils.log('Failure during cache write: %s' % (e))
    ...
    def wrap(func):
        ...
        def memoizer(*args, **kwargs):
            ...
            if in_cache:
                ...
                utils.log('Using method cache for: |%s| -> |%d|' % (full_name, len(pickle.dumps(result))))
                return result
            else:
                # log_utils.log('Calling cached method: |%s|%s|%s|' % (full_name, args, kwargs), log_utils.LOGDEBUG)
                utils.log('Calling cached method: |%s|' % (full_name))
    ...
    def wrap(func):
        ...
        def memoizer(*args, **kwargs):
            ...
            if in_cache:
                ...
                utils.log('Using function cache for: |%s| -> |%d|' % (name, len(pickle.dumps(result))))
                return result
            else:
                # log_utils.log('Calling cached function: |%s|%s|%s|' % (name, args, kwargs), log_utils.LOGDEBUG)
                utils.log('Calling cached function: |%s|' % (name))
                result = func(*args, **kwargs)
                if cache_enabled and cache_limit > 0:
